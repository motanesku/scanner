...
def refresh_universe_endpoint():
    """Forțează re-descărcarea ticker universe din Polygon (cu SIC codes)"""
    from app.data.ticker_universe import build_universe
    u = build_universe(force=True)
    tickers = u.get("tickers", {})
    # Verifică dacă SIC e prezent în noul universe
    has_sic = any(
        info.get("sic_code")
    ...
    sample = [
        ...
        for t, info in list(tickers.items())[:5]
    ]
    return {
        "status": "ok",
        "count": u.get("count", 0),
...
def debug_universe():
    """Statusul ticker universe curent — include verificare SIC"""
...
def debug_polygon():
    ...
    return collect_market_data(["NVDA", "AMD", "TSLA"])
...
def debug_insider_raw():
    ...
    return {"total_filings_found": len(filings), "sample_parsed": results}
...
def debug_earnings_raw():
    ...
    return {"count": len(results), "sample": dict(list(results.items())[:10])}
...
def debug_insider():
    ...
    return {"count": len(triggers), "sample": triggers[:5]}
        ...
        try:
            ...
            if r.ok:
                ...
                try:
                    ...
                    results[name] = {"status": r.status_code, "raw_start": raw, "parse": f"ERROR: {e}"}
            else:
